(function () {
  ...
  function readIntegrationPayload() {
    ...
    if (raw) {
      try {
        ...
        var total =
          typeof o.totalBrl === "number"
            ? o.totalBrl
            : parseFloat(String(o.totalBrl).replace(",", "."));
    ...
    if (only) {
      ...
      if (Number.isFinite(n) && n > 0)
        return { totalBrl: n, titulo: null, local: null };
    }
  ...
  function fillInstallments(select, totalValue) {
    ...
    for (var i = 1; i <= maxParcelas; i++) {
      ...
      opt.textContent =
        i === 1
          ? "1x de " + eachFmt + " (sem juros)"
          : i + "x de " + eachFmt + " (sem juros)";
      select.appendChild(opt);
    }
  }
